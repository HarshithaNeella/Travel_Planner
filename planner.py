...
def handle_message(user_input: str, memory: TravelMemory) -> str:
    ...
    # 🔥 FORCE planning if enough data is available
    if all([
        extracted.get("destination") or state.get("destination"),
        extracted.get("days") or state.get("days"),
        extracted.get("budget") or state.get("budget"),
    ]):
        intent = "planning"
